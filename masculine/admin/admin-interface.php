...
<div class="wrap" id="of_container">
    ...
    <form id="of_form" method="post" action="<?php echo esc_attr($_SERVER['REQUEST_URI']) ?>"
        ...
        <div id="info_bar">
            ...
            <img style="display:none" src="<?php echo ADMIN_DIR; ?>images/loading-bottom.gif"
        ...
        <div class="save_bar">
            ...
            <button id="of_reset" type="submit"
                    class="button submit-button reset-button"><?php _e('Options Reset');?></button>
        </div>
        <!--.save_bar-->
...
jQuery(document).ready(function ($) {
        ...
        styleSelect = {
            init:function () {
                $('.select_wrapper').each(function () {
                    ...
                    if ($(this).parent().hasClass("pslider")) $(this).find('span').html("Add Slide");
                });
    ...
    $(".slide_add_button").live('click', function () {
        ...
        var numArr = $('#' + sliderId + ' li').find('.order').map(
            function () {
                var str = this.id;
                str = str.replace(/\D/g, '');
                str = parseFloat(str);
    ...
    //----------------------------------------------------------------*/
    // Collision Info Columns MOD
    //----------------------------------------------------------------*/


    //Hide (Collapse) the toggle containers on load
    $(".infocolumn_body").hide();

    //Switch the "Open" and "Close" state per click then slide up/down (depending on open/close state)
    $(".infocolumn_edit_button").live('click', function () {
        $(this).parent().toggleClass("active").next().slideToggle("fast");
        return false; //Prevent the browser jump to the link anchor
    });

    // Update slide title upon typing
    function update_infocolumn_title(e) {
        var element = e;
        if (this.timer) {
            clearTimeout(element.timer);
        }
    ...
    $('.of-infocolumns-title').live('keyup', function () {
        update_infocolumn_title(this);
    });


    /* Remove individual slide */

    $('.infocolumn_delete_button').live('click', function () {
        // event.preventDefault();
        var agree = confirm("Are you sure you wish to delete this column?");
        if (agree) {
            var $trash = $(this).parents('li');
            //$trash.slideUp('slow', function(){ $trash.remove(); }); //chrome + confirm bug made slideUp not working...
            $trash.animate({
    ...
    $(".infocolumn_add_button").live('click', function () {
        var infocolumnsContainer = $(this).prev();
        var sliderId = infocolumnsContainer.attr('id');
        var sliderInt = $('#' + sliderId).attr('rel');
        ...
        var newSlide = '<li class="temphide"><div class="slide_header"><strong>Info ' + newNum + '</strong><input type="hidden" class="infocolumn of-input order" name="' + sliderId + '[' + newNum + '][order]" id="' + sliderId + '_infocolumn_order-' + newNum + '" value="' + newNum + '"><a class="infocolumn_edit_button" href="#">Edit</a></div><div class="infocolumn_body" style="display: none; "><label>Title</label><input class="infocolumn of-input of-infocolumns-title" name="' + sliderId + '[' + newNum + '][title]" id="' + sliderId + '_' + newNum + '_infocolumn_title" value=""><label>Description</label><textarea class="infocolumn of-input" name="' + sliderId + '[' + newNum + '][description]" id="' + sliderId + '_' + newNum + '_infocolumn_description" cols="8" rows="8"></textarea><label>Width</label><div class="select_wrapper"><span>100%</span><select class="select" name="' + sliderId + '[' + newNum + '][width]" ><option value="twelvecol" selected="selected">100%</option><option value="sixcol">50%</option><option value="fourcol">33%</option><option value="threecol">25%</option></select></div><label>Link Name (optional)</label><input class="infocolumn of-input" name="' + sliderId + '[' + newNum + '][link]" id="' + sliderId + '_' + newNum + '_infocolumn_link" value=""><label>Link Url (optional)</label><input class="infocolumn of-input" name="' + sliderId + '[' + newNum + '][url]" id="' + sliderId + '_' + newNum + '_infocolumn_url" value=""><a class="infocolumn_delete_button" href="#">Delete</a><div class="clear"></div></div></li>';

        infocolumnsContainer.append(newSlide);
        $('.temphide').fadeIn('fast', function () {
            $(this).removeClass('temphide');
        });
    ...
    // Sort Slides
    jQuery('.infocolumns').find('ul').each(function () {
        var id = jQuery(this).attr('id');
        $('#' + id).sortable({
            placeholder:"placeholder",
            opacity:0.6
    ...
    /* Remove individual slide */
    $('.pslide_delete').live('click', function () {
        // event.preventDefault();
        var agree = confirm("Are you sure you wish to delete this slide?");
        if (agree) {
            var $trash = $(this).parents('li');
    ...
    $('.pslider .select ').prop('selectedIndex', -1);
    $('.pslider .select_wrapper  span ').html('Add Slide');

    /* Add new slide */
    $(".pslider .select").change(function () {

        var _id = $(".pslider ul").attr('id');
        var _name = $('.pslider .select option:selected').html();
        var _val = $(this).val();
//        var _order = $(".pslider ul li").length;
        var numArr = $('.pslider ul li').find('.order').map(
            function () {
                var str = this.id;
                str = str.replace(/\D/g, '');
                str = parseFloat(str);
        ...
        var newSlide = '<li><a class="pslide_delete" href="#">Delete</a><div class="pslide_header"><strong>' + _name + '</strong>' +
            '<input type="hidden" class="pslide of-input order" name="' + _id + '[' + _order + '][order]" id="' + _id + '_' + _order + '_pslide_order" value="' + _order + '" />' +
            '<input type="hidden" class="pslide of-input id" name="' + _id + '[' + _order + '][id]" id="' + _id + '_' + _order + '_pslide_id" value="' + _val + '" />' +
            '</div><div class="clear"></div></li>';

        $(".pslider ul").append(newSlide);

    });

    // Sort Slides
    jQuery('.pslider').find('ul').each(function () {
        var id = jQuery(this).attr('id');
        $('#' + id).sortable({
            placeholder:"placeholder",
            opacity:0.6
    ...
    //----------------------------------------------------------------*/
    // sQrt121 Home Checkboxes
    //----------------------------------------------------------------*/
    /* Flex Slider Check */
    $("#section-iter_home_fslider_state input[type=checkbox]").click(function (e) {
        checkFSlider();
    });
    function checkFSlider() {
        if ($("#section-iter_home_fslider_state input[type=checkbox]").attr('checked') != undefined) {
            $("#section-iter_home_fslider").show();
        } else {
            $("#section-iter_home_fslider").hide();
        }
    }

    checkFSlider();
    /* Portfolio Slider Check */
    $("#section-iter_home_pslider_state input[type=checkbox]").click(function (e) {
        checkPSlider();
    });
    function checkPSlider() {
        if ($("#section-iter_home_pslider_state input[type=checkbox]").attr('checked') != undefined) {
            $("#section-iter_home_pslider").show();
        } else {
            $("#section-iter_home_pslider").hide();
        }
    }
    ...
    /* Info Columns check */
    $("#section-iter_home_infocolumns_state input[type=checkbox]").click(function (e) {
        checkInfoColumn();
    });
    function checkInfoColumn() {
        if ($("#section-iter_home_infocolumns_state input[type=checkbox]").attr('checked') != undefined) {
            $("#section-iter_home_infocolumns").show();
        } else {
            $("#section-iter_home_infocolumns").hide();
        }
    }
    ...
    /* Flex Slider Check */
    $("#section-iter_home_clients_state input[type=checkbox]").click(function (e) {
        checkClients();
    });
    function checkClients() {
        if ($("#section-iter_home_clients_state input[type=checkbox]").attr('checked') != undefined) {
            $("#section-iter_home_clients").show();
        } else {
            $("#section-iter_home_clients").hide();
        }
    }
    checkClients();
    /*
        ...
        foreach ($options as $value) {
            ...
            switch ($value['type']) {
                ...
                case 'infocolumns':
                    ...
                    $int = '';
                    $int = optionsframework_mlu_get_silentpost($_id);
                    $output .= '<div class="infocolumns"><ul id="' . $value['id'] . '" rel="' . $int . '">';
					  if (isset($data[$value['id']])) {
                    $infocolumnns = $data[$value['id']];
                    $count = count($infocolumnns);
                    if ($count < 2) {
                        $oldorder = 1;
                        $order = 1;
                        $output .= Options_Machine::optionsframework_infocolumns_function($value['id'], $value['std'], $oldorder, $order, $int);
                    } else {
                        $i = 0;
                        foreach ($infocolumnns as $infocolumnn) {
                            $oldorder = $infocolumnn['order'];
                            $i++;
                            $order = $i;
                            $output .= Options_Machine::optionsframework_infocolumns_function($value['id'], $value['std'], $oldorder, $order, $int);
                        }
                    ...
                    $output .= '</ul>';
                    $output .= '<a href="#" class="button infocolumn_add_button">Add New Column</a></div>';
                ...
                case 'pslider':
                    ...
                    $int = '';
                    $int = optionsframework_mlu_get_silentpost($_id);
                    $output .= '<div class="pslider"><ul id="' . $value['id'] . '" rel="' . $int . '">';
                    if (isset($data[$value['id']])) {
                        $pslider = $data[$value['id']];
                        //
                        $count = count($pslider);
                        if ($count < 2) {
//
                            ...
                            $i = 0;
                            foreach ($pslider as $pslide) {
                                $i++;
                                $oldorder = $pslide['order'];
                                $order = $i;
                                $output .= Options_Machine::optionsframework_pslider_function($value['id'], $value['std'], $oldorder, $order, $int);

                            }
                        }
                    }
                    $output .= '</ul>';
                    $output .= '<div class="select_wrapper"><select class="select" name="">';
    ...
    /*-----------------------------------------------------------------------------------*/
    /* Collision Info Columns - optionsframework_infocolumns_function */
    /*-----------------------------------------------------------------------------------*/

    public static function optionsframework_infocolumns_function($id, $std, $oldorder, $order, $int)
    {
        ...
        if (isset($infocolumn[$oldorder])) {
            $val = $infocolumn[$oldorder];
        } else {
            $val = $std;
        }

        //initialize all vars
        $infocolumnvars = array('title', 'description', 'width', 'link', 'url');

        foreach ($infocolumnvars as $infocolumnvar) {
            if (!isset($val[$infocolumnvar])) {
                $val[$infocolumnvar] = '';
            }
        }
        ...
        //begin slider interface
        if (!empty($val['title'])) {
            $infocolumns .= '<li><div class="slide_header"><strong>' . stripslashes($val['title']) . '</strong>';
        } else {
            $infocolumns .= '<li><div class="slide_header"><strong>Info ' . $order . '</strong>';
        }

        $infocolumns .= '<input type="hidden" class="infocolumn of-input order" name="' . $id . '[' . $order . '][order]" id="' . $id . '_' . $order . '_infocolumn_order" value="' . $order . '" />';

        $infocolumns .= '<a class="infocolumn_edit_button" href="#">Edit</a></div>';

        $infocolumns .= '<div class="infocolumn_body">';

        $infocolumns .= '<label>Title</label>';
        $infocolumns .= '<input class="infocolumn of-input of-infocolumns-title" name="' . $id . '[' . $order . '][title]" id="' . $id . '_' . $order . '_infocolumn_title" value="' . stripslashes($val['title']) . '" />';

        $infocolumns .= '<label>Description</label>';
        $infocolumns .= '<textarea class="infocolumn of-input" name="' . $id . '[' . $order . '][description]" id="' . $id . '_' . $order . '_infocolumn_description" cols="8" rows="8">' . stripslashes($val['description']) . '</textarea>';

        $infocolumns .= '<label>Width</label>';
        $infocolumns .= '<div class="select_wrapper">
        <select class="select" name="' . $id . '[' . $order . '][width]">
                            <option value="twelvecol"' . selected($val['width'], 'twelvecol', false) . '>100%</option>
                            <option value="sixcol"' . selected($val['width'], 'sixcol', false) . '>50%</option>
                            <option value="fourcol"' . selected($val['width'], 'fourcol', false) . '>33%</option>
                            <option value="threecol"' . selected($val['width'], 'threecol', false) . '>25%</option>
        </select></div>';

        $infocolumns .= '<label>Link Name (optional)</label>';
        $infocolumns .= '<input class="infocolumn of-input" name="' . $id . '[' . $order . '][link]" id="' . $id . '_' . $order . '_infocolumn_link" value="' . $val['link'] . '" />';

        $infocolumns .= '<label>Link URL (optional)</label>';
        $infocolumns .= '<input class="infocolumn of-input" name="' . $id . '[' . $order . '][url]" id="' . $id . '_' . $order . '_infocolumn_url" value="' . $val['url'] . '" />';


        $infocolumns .= '<a class="infocolumn_delete_button" href="#">Delete</a>';
        $infocolumns .= '<div class="clear"></div>' . "\n";
    ...
    /*-----------------------------------------------------------------------------------*/
    /*-----------------------------------------------------------------------------------*/
    /* sqrt121 pslider - optionsframework_infocolumns_function */
    /*-----------------------------------------------------------------------------------*/

    public static function optionsframework_pslider_function($id, $std, $oldorder, $order, $int)
    {
        ...
        $pslider .= '<li><a class="pslide_delete" href="#">Delete</a><div class="pslide_header"><strong>' . get_the_title($val['id']) . '</strong>';
        $pslider .= '<input type="hidden" class="pslide of-input order" name="' . $id . '[' . $order . '][order]" id="' . $id . '_' . $order . '_pslide_order" value="' . $order . '" />';
        $pslider .= '<input type="hidden" class="pslide of-input id" name="' . $id . '[' . $order . '][id]" id="' . $id . '_' . $order . '_pslide_id" value="' . stripslashes($val['id']) . '" />';


        $pslider .= '</div>';
        $pslider .= '<div class="clear"></div>' . "\n";
        $pslider .= '</li>';
